	...
	private void initialize() {
		...
		btnHealthCareProvider_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				PreparedStatement pst = null;
				try{
					String query="select * from userInfo where ID=? and password=? and Username=?";
					pst= connection.prepareStatement(query);
					...
					if(count == 1){
						...
						JOptionPane.showMessageDialog(null, "Duplicates of Username, password, or HCPID");
					} else{
						JOptionPane.showMessageDialog(null, "Username, password, or HCPID are not correct");
					}
